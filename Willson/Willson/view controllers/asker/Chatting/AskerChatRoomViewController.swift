...
//  Willson
//
//  Created by JHKim on 2019/11/15.
//
...
//    var lastSection: Int?
    ...
    @IBOutlet weak var clockBarButtonItem: UIBarButtonItem!
    ...
    // MARK: - IBAction
    @IBAction func tappedClockBarButtonItem(_ sender: Any) {
        guard let vc: AskerChatReviewViewController = UIStoryboard(name: "AskerChat", bundle: nil).instantiateViewController(withIdentifier: "AskerChatReviewViewController") as? AskerChatReviewViewController else {
            print("tappedClockBarBUttonItem: AskerChatReviewViewController 할당 오류")
            return
        }
        guard let chatRoomDataRow = chatRoomDataRow else {
            print("tappedClockBarButtonItem: chatRoomDataRow 할당 오류")
            return
        }
    ...
    override func viewDidLoad() {
        ...
        if let nickname = chatRoomDataRow?.willsoner?.asker.nickname {
...
//            let indexPath = IndexPath(row: self.lastRowInLastSection!, section: self.lastSection!)
    ...
    func timeLimitStop() {
        ...
        // show askerChatReview
        guard let vc: AskerChatReviewViewController = UIStoryboard(name: "AskerChat", bundle: nil).instantiateViewController(withIdentifier: "AskerChatReviewViewController") as? AskerChatReviewViewController else { return }
        vc.chatRoomDataRow = chatRoomDataRow
    ...
    func startMsgListener() {
        ...
        startListener = db.collection("chatrooms").document(roomkey).collection("chats").document("start").addSnapshotListener { querySnapshot, error in
            ...
            guard let data = snapshot.data() else {
                print("StartMsg: snapshot.data() 할당 오류")
                return
            }
    ...
    func checkAgreement() {
        guard let roomkey = self.roomkey else {
            print("checkAgreement: roomkey 할당 오류")
            return
        }
    ...
    func introMsgListener() {
        guard let roomkey = roomkey else {
            print("introMsgListener: roomkey 할당 오류")
            return
        }
